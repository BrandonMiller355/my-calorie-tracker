...
describe('computeWeeklyDeficit', () => {
  it('sums (goal - consumed) across the week so far, excluding today', () => {
    const week = [
    ...
    // (2000-1800) + (2200-2500) = 200 - 300 = -100; today (07-08) is excluded
    expect(result.deficit).toBe(-100);
  });

  it('rounds the deficit to one decimal place', () => {
    const week = [day({ date: '2026-07-05', effectiveGoalCalories: 2000.05, consumedCalories: 1800.02 })];
    const result = computeWeeklyDeficit(week, '2026-07-06', '2026-07-06');
    expect(result.deficit).toBe(200.0);
  });

  it('handles the Monday-only case (single day in range) when that day is not today', () => {
  ...
  it('excludes today from the deficit even when it is the only day in range', () => {
    const week = [day({ date: '2026-07-06', effectiveGoalCalories: 2000, consumedCalories: 1200 })];
    const result = computeWeeklyDeficit(week, '2026-07-06', '2026-07-06');
