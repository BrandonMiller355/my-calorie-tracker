# ai-text-logging Specification

## Purpose
Log one or more foods from a single free-text description ("2 slices of sara lee bread with 1 serving of pbfit") — dictated via the phone keyboard's dictation key or typed — by parsing it with an AI model against the food library into proposed entries (library matches with amounts, or nutrition estimates for unknown foods), reviewing them, and bulk-logging. A single parsed item prefills the entry form instead, reusing the existing combobox-pick and AI-estimate flows.
